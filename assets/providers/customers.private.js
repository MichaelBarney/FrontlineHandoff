...
const formatCustomerRecord = (customerRecord) => {
  try {
    const unformattedAddress = customerRecord.get("Contact");
    const formattedAddress = unformattedAddress.replace(/[-()]/gm, "");

    return {
      customer_id: `${customerRecord.get("id")}`,
      display_name: `${customerRecord.get("Candidate")}`,
      position: `${customerRecord.get("Position")}`,
      channels: [
        { type: "sms", value: `${customerRecord.get("Contact")}` },
        {
          type: "whatsapp",
          value: `whatsapp:${customerRecord.get("Contact")}`,
        },
      ],
      links: [
        {
...
const getCustomersList = async (context, worker, pageSize, anchor) => {
  ...
  console.log("Test 2: ", worker, anchor, customers.length);
...
const getCustomerByNumber = async (context, customerNumber) => {
  const customer = await getAirtableCustomerByParams(context, {
    view: "Grid view",
    filterByFormula: `{Contact} = '${customerNumber}'`,
    maxRecords: 1,
  });
  return customer;
};
